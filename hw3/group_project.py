...
"hang dong"
--description
"multi-objectives classification for hotels based on Tripadvisor reviews. The labels include'service', 'location', 'environment', 'facility', 'food'"
--readfile
label.data
--emails
donghang@usc.edu
...
class classifier:
    ...
    def add_entry(self, data):
        ...
        """
        #tuple1=(data, label)
        self.list_data["corpus"].append({"data":data})
...
def main():
    ...
    if args.readfile is not None:
        ...
        for line in f:
            ...
            #if len(tup) != 2:
            #    print(str(count)+" line is not valid")
            cls.add_entry(*tup)
            count +=1
